...
def training(num_episodes, env, dqn, optimizer, criterion, gamma, epsilon, epsilon_min, epsilon_decay, action_size, save_path, scheduler=None):
    ...
    for episode in tqdm(range(num_episodes), unit='episodes', unit_scale=True, desc='Training'):
        ...
        while True:
            ...
            if env.current_player == -1:
                # Only train on the agent's actions
...
def test(dqn, num_games=50):
    ...
    results_vs_random = {'wins': 0, 'losses': 0, 'draws': 0}
    results_vs_self = {'player1_wins': 0, 'player2_wins': 0, 'draws': 0}
    
    with torch.no_grad():
        # Play against a random agent
        for _ in tqdm(range(num_games), desc='Testing vs Random'):
            state = env.reset()
            while not env.done:
                if env.current_player == 1:
                    state_tensor = torch.FloatTensor(state).unsqueeze(0)
            ...
            if reward == 1 and env.current_player == -1:
                results_vs_random['wins'] += 1
            elif reward == 1 and env.current_player == 1:
                results_vs_random['losses'] += 1
            else:
                results_vs_random['draws'] += 1
            wandb.log({"vs_random": results_vs_random})
    ...
    print(f"  Wins: {results_vs_random['wins']}")
    print(f"  Losses: {results_vs_random['losses']}")
    print(f"  Draws: {results_vs_random['draws']}")
